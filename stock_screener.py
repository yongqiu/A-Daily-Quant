...
def check_market_risk() -> bool:
    ...
    try:
        ...
        if last["close"] < last["ma20"] and last["change_pct"] < -1.0:
            print(
                "🛑 RISK ALERT: Market Risk Logic Triggered. Market below MA20 and dropping > 1%. Strategy Halted."
            )
            return True
...
def run_rough_screen(
    df: pd.DataFrame, criteria: Dict[str, Any]
) -> List[Dict[str, Any]]:
    """
    执行步骤 1：基于基本指标的粗略筛选
    ...
    if "sector" not in df.columns:
        ...
        if sector_map:
            ...
            print(
                "   ⚠️ No sector info available (Source missing & No local map). Skipping rank_in_sector."
            )
    ...
    if "sector" in df.columns and "change_pct" in df.columns:
        ...
        df["rank_in_sector"] = df.groupby("sector")["change_pct"].rank(
    ...
    if "turnover_rate" in result_df.columns:
        ...
        result_df = result_df.sort_values(
            by=["turnover_rate", "change_pct"], ascending=[False, False]
        )
    else:
...
def run_deep_screen(
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_stock = {
            executor.submit(analyze_candidate, c, config): c for c in candidates
        }
...
def run_stock_selection(config: Dict[str, Any]):
    ...
    print(
        f"📋 Current Selection Rules: {json.dumps(rules, indent=2, ensure_ascii=False)}"
    )
    ...
    print(
        f"📊 Market Mood: {mood_data['market_mood']} (Up: {mood_data['up_count']}, LimitUp: {mood_data['limit_up']})"
    )
