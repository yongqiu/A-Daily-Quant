...
- **形态**：{{ computed.ma_str }}
- **趋势**：MA60(牛熊线): {{ computed.ma60 }}
- **动量**：MACD: {{ computed.macd_str }} | RSI: {{ computed.rsi }}
- **位置**：上方压力 {{ computed.res }} / 下方支撑 {{ computed.sup }}
- **资金**：{{ computed.funds_str }}
- **量能**：量比 {{ computed.vol_ratio }}
- **板块**：{{ computed.sector }} ({{ computed.sector_change }}%)

**二、明日推演**
1. **评分解读**：当前评分 {{ tech_data.composite_score }}，结合技术面判断强弱。
2. **多空博弈**：根据量价配合及资金流向推测主力意图（试盘/出货/吸筹）。
3. **情景预演**：
    - **情景A（向上突破）**：关键点位及确认信号。
    - **情景B（向下调整）**：低吸机会或破位风险。
