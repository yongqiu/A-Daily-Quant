...
def get_latest_candidate_file() -> Optional[str]:
    ...
    files.sort(
        key=lambda x: re.search(CANDIDATES_FILE_PATTERN, x).group(1), reverse=True
    )
...
def parse_candidates_from_md(filename: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for symbol, name in matches:
            candidates.append(
                {"symbol": symbol, "name": name.strip(), "type": "candidate"}
            )
...
def get_realtime_data(targets: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # Group by asset type
    stocks = [
        t["symbol"] for t in targets if t.get("asset_type") in ["stock", "etf", None]
    ]
    ...
    if stocks:
        try:
            ...
            print(
                f"🔄 [MonitorEngine] Batch fetching {len(stocks)} stocks via Tushare..."
            )
            ts_df = ts.get_realtime_quotes(stocks)
            if ts_df is not None and not ts_df.empty:
                print(
                    f"✅ [MonitorEngine] Tushare batch returned {len(ts_df)} records."
                )
    ...
    if missing_stocks:
        print(
            f"🔄 [MonitorEngine] FALLBACK: Fetching {len(missing_stocks)} missing stocks via Tencent..."
        )
        try:
            query_list = []
            ...
            print(
                f"✅ [MonitorEngine] Tencent fallback fetched {len(results) - (len(stocks) - len(missing_stocks))} records."
            )
        except Exception as e:
...
def get_realtime_rules(asset_type: str = "stock") -> Dict[str, Any]:
    ...
    rules = DEFAULT_STRATEGY_RULES.get(
        asset_type, DEFAULT_STRATEGY_RULES["stock"]
    ).copy()
...
def check_strategy(
    stock_data: Dict[str, Any], strategy_type: str = "general"
) -> Dict[str, Any]:
    """
    ...
    return {"status": status, "alerts": alerts, "raw_data": stock_data}
...
class MonitorEngine:
    def __init__(self):
        ...
        self.market_history_cache = {"data": None, "timestamp": None}
    ...
    def refresh_targets(self):
        ...
        if "watch_list" in monitor_config:
            for w in monitor_config["watch_list"]:
                ...
                if asset_type == "etf":
                    asset_type = "etf"  # explicit check if needed
        ...
        if "portfolio" in self.config:
            for p in self.config["portfolio"]:
                existing = next(
                    (t for t in new_targets if t["symbol"] == p["symbol"]), None
                )
        ...
        try:
            ...
            for h in db_holdings:
                existing = next(
                    (t for t in new_targets if t["symbol"] == h["symbol"]), None
                )
    ...
    def run_check(self):
        ...
        if target_symbols:
            daily_metrics_map = database.get_daily_metrics_batch(
                target_symbols, today_str
            )
        else:
            daily_metrics_map = {}
                ...
                results.append(
                    {
                        ...
                        "name": target["name"],
                        "type": target["type"],
                        "asset_type": target.get(
                            "asset_type", "stock"
                        ),  # Missing asset_type in fallback
                        "cost_price": target.get("cost_price", 0),
                        "position_size": target.get("position_size", 0),
                        "status": "offline",
                        "alerts": ["无数据"],
